...
select Groups.gid, group_name, description from groups, members where '1' = Members.uid and Members.gid = Groups.gid;

--example, selecting all the messages from the group ordered by time
select txt from messages where 'd' = gid order by m_date, m_time asc;

--example selecting all events for a given group
select * from events where 'a' = gid;

--selecting all members in a given group
select * from members where 'a' = gid;
